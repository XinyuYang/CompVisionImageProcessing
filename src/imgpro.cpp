...
"  -sobelY\n"
"  -log\n"
"  -bilateral <real:blurSigma> <real:tolerance>\n"
"  -median <real:kernelSize> <real:sigma>\n"
"  -harris <real:sigma>\n"
...
main(int argc, char **argv)
{
  ...
  while (argc > 0) {
    ...
    else if (!strcmp(*argv, "-scaleInvariantHarris")){
      argv++, argc--;
      image->ScaleInvariantHarris();
    }
    else if (!strcmp(*argv, "-bilateral")){
      CheckOption(*argv, argc, 3);
      double blurSigma = atof(argv[1]);
      double tolerance = atof(argv[2]);
      argv += 3, argc -= 3;
      image->Bilateral(blurSigma, tolerance);
    }
